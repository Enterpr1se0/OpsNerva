...
export function AuditRunsView({history,hosts,notify}:{history:AuditHistoryStore;hosts:Host[];notify:NotificationSink}){
	...
	useEffect(()=>{history.setQuery(settledQuery)},[history,settledQuery])
	...
	return <div className="audit-runs-view page-stack">
		<div className="audit-toolbar">
			<div className="search-box"><Search size={16}/><input aria-label={t('common.search')} value={query} onChange={event=>setQuery(event.target.value)}/></div>
		...
		{page.ready&&page.loading==='idle'&&!page.error&&!page.items.length&&<Empty icon={view.query?<Search/>:<History/>} title={t(view.query?'audit.noMatch':'audit.emptyTitle')}/>}
