const zh = {
  ...
  skills: {
    uploaded: '{{name}} 已上传并启用。', uploadedMany: '已上传并启用 {{count}} 个 Skills。', saved: '{{name}} 已保存。',
	    ...
	    title: 'Skills',
	    reload: '重载', uploadSkill: '上传 Skill', uploadPackage: '上传 Markdown、ZIP 或 7z 包',
	    skillName: 'Skill 名称', invalidName: 'Skill 名称格式无效。', choosePackage: '选择 .md、.zip 或 .7z',
  ...
  settings: {
	...
	contextCompression: '上下文压缩', autoCompression: '自动压缩', compressionThreshold: '触发阈值（%）', compressionOff: '已关闭',
		chatImages: '会话图片格式', desktopApplication: '桌面应用', lightweightMode: '轻量模式', openDeveloperTools: '打开开发者工具',
  ...
  mcpServerMode: {
    ...
    start: '启动', stop: '停止', rotate: '重新生成令牌', activity: '调用记录',
  ...
  tool: {
    ...
    permission: '权限', sandbox: 'Bubblewrap 沙盒', exitCode: '退出码', duration: '耗时', runId: '运行 ID',
    reason: '操作目的', result: '工具结果', operatorInstruction: '操作员指令',
    rawJson: '原始 JSON', diff: '文件差异', fileEdit: '编辑文件', index: '序号', key: '键', value: '值', stderrResult: 'STDERR / 结果', outputOmitted: '省略 {{count}} B', previewOmitted: '省略 {{count}} 个字符', previewItemsOmitted: '省略 {{count}} 项', moreItemsOmitted: '其余项目已省略', waitDeadline: '等待截止，任务继续运行', moreOutput: '还有输出',
    taskStatus: '后台任务状态', taskCancel: '取消后台任务', liveTask: '实时',
    matchModeLiteral: '字面量', matchModeRegex: '正则表达式', searchMatched: '已找到匹配项', searchNoMatches: '未找到匹配项', transferProgress: '传输进度',
  },
  approval: {
    actionScript: '脚本', actionCommand: '命令', sudoFileTitle: '是否允许 root 修改系统文件', sudoTitle: '是否允许以 root 权限执行{{kind}}',
    ...
    pendingOperation: '待审批操作', approved: '审批已通过 · {{status}} · {{run}}', autoApproved: 'Auto 允许',
  ...
  hosts: {
    authAgent: 'SSH Agent', authKey: '使用私钥', authPassword: '账户密码', sudoNone: '仅 user', sudoNopasswd: 'root（无需密码）', sudoPassword: 'root（托管密码）',
    ...
    proxyJump: 'SSH 跳板机', direct: '直连', proxy: '代理', knownHosts: 'Known hosts 文件', useDefault: '使用系统默认值', sudoPolicy: 'root 权限', sudoPasswordLabel: 'root 密码',
  ...
  models: {
    ...
    modelPlaceholder: '获取模型或手动输入 ID', apiKey: 'API Key', keepKey: '留空以保留当前 Key',
		reasoningEffort: '思考程度', contextWindow: '上下文窗口', contextAuto: '自动', contextRange: '请输入 1,024–10,000,000',
    baseUrl: 'Base URL', officialEndpoint: '留空使用 OpenAI 官方地址',
    userAgent: 'User-Agent', userAgentHint: '留空使用 SDK 默认值；网关拒绝 SDK 请求时可填写自定义 UA',
		proxy: '代理', noProxy: '直连',
  ...
  audit: {
    ...
    counts: '{{sessions}} 个会话 · {{runs}} 次运行', noSession: '没有 Agent 会话上下文', lastRun: '最近运行 {{date}}', pending: '待审批',
