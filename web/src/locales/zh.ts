const zh = {
  ...
  chat: {
    ...
    emptyTitle: '需要排查什么？',
    waitingModel: '等待模型响应', retryingModel: '模型请求失败，正在重试', backgroundRunning: '后台运行中，刷新页面不会中断。',
  ...
  approval: {
    ...
    uploadTitle: '是否允许上传文件', transferTitle: '是否允许在 SSH 主机间传输文件', tunnelTitle: '是否允许建立本机 SSH 端口转发', hostShellTitle: '是否允许在宿主机执行脚本', fileTitle: '是否允许修改文件', executeTitle: '是否允许执行{{kind}}',
    uploadLabel: '将要上传的文件', transferLabel: '文件传输', tunnelLabel: '端口转发链路', rootFileLabel: '将以 root 权限修改的文件', rootCommandLabel: '将以 root 权限执行的完整{{kind}}', fileLabel: '将要修改的文件', commandLabel: '将要执行的完整{{kind}}',
    ...
    plainExplanation: '命令说明', risks: '风险', degradedInfo: '错误信息',
    ...
    allowSession: '本会话允许同一操作', hostUnavailable: '宿主机命令不可用', fileReadUnavailable: '文件内容访问每次都需要单独审批', tunnelSessionUnavailable: '建立端口转发每次都需要单独审批', criticalUnavailable: '高风险操作不可用',
  ...
  statusLabels: { pending: '待处理', completed: '已完成', failed: '失败', active: '进行中', running: '运行中', stopping: '停止中', stopped: '已停止', blocked: '已阻塞', in_progress: '执行中', ready: '就绪', error: '错误', connecting: '连接中', interrupted: '已中断', rejected: '已拒绝', approval_required: '待审批' },
  ...
  toolNames: {
    ...
    ssh_task: '管理后台任务', ssh_tunnel: '管理 SSH 端口转发', ssh_host_list: '列出主机', ssh_host_inspect: '检查主机',
