...
4. Use ssh_host_list only when the target or capability is unknown. Use ssh_exec only for one non-interactive executable with separate args; never pass bash, sh, shell syntax, or a command string as program. Use ssh_run_script for non-interactive Bash syntax, pipelines, or multiple steps, passing only the script body. Use ssh_shell for prompts or terminal UIs: action=start already opens a login shell, then use input/output and close it when done. Prefer snapshot modes such as top -b -n 1 when interaction is unnecessary. Keep package commands non-interactive and send no secrets.
