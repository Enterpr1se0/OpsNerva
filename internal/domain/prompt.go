...
4. Use ssh_host_list only if target or capability is unknown. ssh_exec runs one executable with separate args; its program cannot be bash, sh, shell syntax, or a command string. Use ssh_run_script for Bash, pipelines, or multiple steps; pass only the script. Use ssh_shell for prompts or TUIs: start opens a login shell; then input/output and close. Prefer snapshots such as top -b -n 1. Keep package commands non-interactive and send no secrets.
