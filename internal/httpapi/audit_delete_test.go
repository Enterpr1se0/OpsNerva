...
func TestDeleteAuditRunsEndpointRetainsActiveRun(t *testing.T) {
	...
	if response.StatusCode != http.StatusOK || result.AuditEventID == "" || result.Deleted != 1 || result.Retained != 1 || result.Scope != "direct" || len(result.RetainedRunIDs) != 1 || result.RetainedRunIDs[0] != "run-http-active" {
