...
连接 MCP Client 调用一次 `ssh_exec`，然后在 Web 的 **审计 / MCP 活动** 查看调用过程和关联运行。确认 MCP 工具列表包含 `ssh_history`，但只能读取该 MCP 客户端会话创建的运行，也不能批准自己的命令。

## 7. 事务化修改远端配置

向 Agent 输入：

> 检查 demo 主机的 nginx 配置；如果确实需要修改，只调整 server_tokens，并使用 nginx validator。
